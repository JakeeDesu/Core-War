...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/11 19:35:45 by hmhindat          #+#    #+#             */
/*   Updated: 2020/11/28 11:02:29 by hmhindat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../corewar.h"

void	exec_and(t_vm *vm, t_carriage *carr, int *args, int *types)
{
	int value;
	int value1;
	int value2;
	...
	if (check_arg_type(vm, types, carriage))
	{
		if ((jump = get_args(vm, args, types, carriage)))
		{
			if (check_reg_val(carriage->op_id, types, args))
				exec_and(vm, carriage, args, types);

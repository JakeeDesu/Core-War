...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/11 20:12:51 by hmhindat          #+#    #+#             */
/*   Updated: 2020/11/28 11:02:37 by hmhindat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../corewar.h"

void	exec_or(t_vm *vm, t_carriage *carr, int *args, int *types)
{
	int value;
	int value1;
	int value2;
	...
	if (check_arg_type(vm, types, carriage))
	{
		if ((jump = get_args(vm, args, types, carriage)))
		{
			if (check_reg_val(carriage->op_id, types, args))
				exec_or(vm, carriage, args, types);

...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/11 19:47:27 by hmhindat          #+#    #+#             */
/*   Updated: 2020/11/28 11:05:17 by hmhindat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../corewar.h"

void	exec_fork(t_vm *vm, t_carriage *carr, int *args, int *types)
{
	t_carriage	*new_carr;
	...
	if (check_arg_type(vm, types, carriage))
	{
		if ((jump = get_args(vm, args, types, carriage)))
		{
			if (check_reg_val(carriage->op_id, types, args))
				exec_fork(vm, carriage, args, types);
